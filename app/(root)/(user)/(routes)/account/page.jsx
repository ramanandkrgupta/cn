...
const getProfileStats = (session) => ({
  uploads: session?.uploads || 0,
  downloads: session?.downloads || 0,
  reputation: session?.reputation || 0,
  daysLeft: session?.daysLeft || 'Lifetime'
});
...
const getMenuItems = (router, userRole, session) => {
  const baseMenuItems = [
    ...
    },
    {
      ...
      title: "Plans",
      description: userRole === 'PRO' ? "Manage your PRO subscription" : "Upgrade to PRO",
      path: "/account/plans",
      badge: userRole === 'PRO' ? 
    ...
    ? [
        {
          ...
          path: "/dashboard",
          badge: <span className="px-2 py-1 bg-primary/10 text-primary text-xs rounded-full">
            Admin
          </span>
        },
        ...baseMenuItems,
  ...
  return [
    {
      ...
      badge: <span className="px-2 py-1 bg-primary/10 text-primary text-xs rounded-full">PRO</span>
    },
    {
...
export default function Profile() {
  ...
  const stats = getProfileStats(session);

  const menuItems = getMenuItems(router, session?.user?.role, session);
      ...
      <div className="mx-auto px-4 max-w-lg py-6">
        ...
        <div className="block mt-4 p-6 bg-base-300 rounded-lg shadow-lg">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                >
                  {session?.user?.role === 'PRO' && <Sparkles size={12} />}
                  {session?.user?.role || 'FREE'} User
                </span>
                {session?.user?.role === 'PRO' && (
                  <span className="text-xs text-gray-500">
                    {stats.daysLeft === 'Lifetime' ? 'Lifetime' : `${stats.daysLeft} days left`}
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 p-4 bg-base-200 rounded-lg">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-secondary">{stats.downloads}</div>
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="space-y-3">
            {/* Account Section */}
            <div className="mb-4">
              <h3 className="text-sm font-medium text-gray-500 px-2 mb-2">ACCOUNT</h3>
            ...
            {session?.user?.role === 'PRO' && (
              <div className="mb-4">
                ...
                <div className="space-y-2">
                  {getProMenuItems(session.user.role).map((item, index) => (
                    <button
                      key={index}
                      onClick={() => router.push(item.path)}
            ...
            {/* Content Section */}
            <div className="mb-4">
              <h3 className="text-sm font-medium text-gray-500 px-2 mb-2">CONTENT</h3>
              <div className="space-y-2">
            ...
            {/* Preferences Section */}
            <div className="mb-4">
              <h3 className="text-sm font-medium text-gray-500 px-2 mb-2">PREFERENCES</h3>
              <div className="space-y-2">
