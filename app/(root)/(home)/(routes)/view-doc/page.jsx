...
// Other helper functions (formatSubjectCode, formatSemester, etc.) remain unchanged
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        setPosts(data.posts)
        setMetadata({ ...data.meta, total: data.posts.length })
      } catch (error) {
    ...
    <div className="container">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2" onClick={() => router.back()}>
          <button aria-label="Go Back" className="hover:bg-base-300 rounded-full transition-colors">
      ...
      {/* Controls and Stats Section (omitted for brevity) */}
      {/* ... */}
      ...
      <div className="items-center">
        ...
        {posts.length > 0 && posts.length >= (metadata?.total || 0) && (
          <div className="text-center mt-6 text-sm text-gray-500">No more posts available</div>
        )}
      </div>
    </div>
  )
